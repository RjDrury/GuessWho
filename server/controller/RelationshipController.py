...
def get_friends_list(User):
    ...
    current_users_relationships = UserRelationships.query.filter_by(id_1=User.id).all()
    
    for rel in current_users_relationships:
        if rel.relationship == 'friends':
            friends_list.append(User.query.filter_by(id=rel.id_2).first().username)
        elif 'pending' in rel.relationship:
            pending_list.append(User.query.filter_by(id=rel.id_2).first().username)
        else:
            blocked_list.append(User.query.filter_by(id=rel.id_2).first().username)

    current_users_relationships = UserRelationships.query.filter_by(id_2=User.id).all()
...
def add_friend_controller(username_current_user, username_to_add):
    ...
    if user_current_user is not None and user_to_add is not None:
        small_id, big_id = proper_id_order(user_current_user.id, user_to_add.id) 
        if small_id == big_id:
        ...
        for rel in relationship:
            if rel.id_2 == big_id:
                if rel.relationship == (str(user_current_user.id)+' pending '+str(user_to_add.id)): #if user already tried to add
        ...
        if not users_have_rel:
            new_rel = UserRelationships(small_id,big_id,str(user_current_user.id)+' pending '+str(user_to_add.id)) 
            db.session.add(new_rel)
            db.session.commit()
